...
async function main() {
  ...
  if (parsed.help) {
    printBanner('ACTION');
    console.log(`  ${paint('Usage:', ANSI.bold)} auramaxx action <schema|run> [options]`);
    console.log('');
    console.log('  Public wrapper around AuraJS action tools.');
    console.log('  Run it from inside an AuraJS game project.');
    console.log('');
    return;
  }

  delegateToAuraJsProjectCommand('action', parsed.passthrough);
}
