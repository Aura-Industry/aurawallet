...
async function main() {
  ...
  if (parsed.help) {
    printBanner('CHECK');
    console.log(`  ${paint('Usage:', ANSI.bold)} auramaxx check [--json]`);
    console.log('');
    console.log('  Public wrapper around AuraJS project validation.');
    console.log('  Run it from inside an AuraJS game project.');
    console.log(`  ${paint('Examples:', ANSI.dim)}`);
    console.log('    auramaxx check');
    console.log('    auramaxx check --json');
    console.log('');
    return;
  }

  delegateToAuraJsProjectCommand('check', parsed.passthrough);
}
