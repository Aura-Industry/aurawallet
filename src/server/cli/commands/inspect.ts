...
async function main() {
  ...
  if (parsed.help) {
    printBanner('INSPECT');
    console.log(`  ${paint('Usage:', ANSI.bold)} auramaxx inspect export [options]`);
    console.log('');
    console.log('  Public wrapper around AuraJS inspect export tools.');
    console.log('  Run it from inside an AuraJS game project.');
    console.log('');
    return;
  }

  delegateToAuraJsProjectCommand('inspect', parsed.passthrough);
}
