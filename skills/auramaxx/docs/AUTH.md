...
aurawallet auth request --agent-id my-agent --profile strict

# Wait for approval in dashboard, then poll locally (or use --no-wait and poll manually)
aurawallet auth poll <requestId> --secret <secret> --private-key-file /tmp/aura-agent-private.pem
```
...
```bash
aurawallet auth request --agent-id my-agent --profile strict --no-wait
aurawallet auth poll <requestId> --secret <secret> --private-key-file /tmp/aura-agent-private.pem --once
```
